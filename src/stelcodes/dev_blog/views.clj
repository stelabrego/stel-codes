...
(defn footer
  ...
  [:footer
   [:p "made by stel abrego with clojure on occupied "
    (he/link-to
      "https://native-land.ca/maps/territories/meskwahki%c2%b7asa%c2%b7hina-fox/"
      "meškwahki land")]])

(defn window
  [title body]
  [:section.window
   [:div.top (raw (slurp "resources/svg/bars.svg")) [:span.title title]
    (raw (slurp "resources/svg/bars.svg"))] [:div.content body]])

(defn tag-group
  [tags]
  [:p.tags
   (for [tag tags]
     (he/link-to {:class "tag"}
                 (state/tag-name->index-uri tag)
                 (str "#" tag " ")))])
...
(comment
  ...
  :blog-post [:id :date-created :date-updated :sort :status :tags :slug :title
              :subtitle :body]
  :educational-media [:id :date-created :date-updated :sort :status :tags :slug
                      :title :subtitle :body :rating]
  :coding-projects [:id :date-created :date-updated :sort :status :tags :slug
                    :title :subtitle :body :production_uri :repository_uri])
    ...
    (when-not (empty? pages)
      (window title
              (list (->> pages
                         ...
                         (he/unordered-list))
                    (when (> page-count 5)
                      (he/link-to {:class "more-link"} more-uri "more!")))))))
  ...
  (let [indexed-pages (:indexed-pages page)]
    (layout page
            (list (welcome-section)
                  (window (:title page)
                          (he/unordered-list (map window-list-item
                                               indexed-pages)))))))
